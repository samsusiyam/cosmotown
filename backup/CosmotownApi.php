...
class CosmotownApi {
    ...
    public function lockDomain($params) {
        ...
        try {
            ...
            $writeResult = @file_put_contents($cacheFile, json_encode($cache));
            $this->cosmotown->logDebug('lockDomain_cache', [
                'file' => $cacheFile,
    ...
    public function getInfo($params) {
        ...
        if (file_exists($cacheFile)) {
            ...
            if (is_array($cache) && isset($cache[$domain]) && (time() - $cache[$domain]['time']) < 60) {
                $data = $this->cosmotown->getDomainInfo($domain, true);
                $locked = $cache[$domain]['locked'];
                if (isset($data['domain'])) {
                    $data['domain']['locked'] = $locked;
                } else {
                    $data['domain'] = ['locked' => $locked];
                }
                $data['locked'] = $locked;
